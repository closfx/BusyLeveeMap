...
function TouchControls(container, camera, options) {
	...
	// Events:
	//
	function onContextMenu(event) {q
		event.preventDefault();
	};

	function onMouseDown(event) {

	    if (self.enabled && event.button === 2 && $('#btn-toggleControls.button-wrap').hasClass('button-active')) {
	...
	function onMouseMove(event) {
	    ...
	    self.mouse.x = (event.clientX / mapWidth) * 2 - 1;
	    self.mouse.y = -(event.clientY / mapHeight) * 2 + 1;
	...
	function onKeyDown(e) {
		...
		switch (e.keyCode) {
		    ...
		    case 81: // rotate left
		        if ($('#btn-toggleControls.button-wrap').hasClass('button-active')) {
		            self.fpsBody.rotation.y += 0.125;
		        }
		        break;

		    case 69: // rotate right
		        if ($('#btn-toggleControls.button-wrap').hasClass('button-active')) {
		            self.fpsBody.rotation.y -= 0.125;
		        }
		        break;
	...
	self.update = function() {
        ...
        if (moveForward && !lockMoveForward || moveBackward && !lockMoveBackward) {

            $('#overlayDefault.overlay').addClass('hidden');
            $('#overlay00.overlay').addClass('hidden');
            $('#overlay01.overlay').addClass('hidden');
            $('#overlay02.overlay').addClass('hidden');
            $('#overlay03.overlay').addClass('hidden');
            $('#overlay04.overlay').addClass('hidden');
        ...
        if (moveLeft && !lockMoveLeft || moveRight && !lockMoveRight) {
        ...
        if (moveForward && lockMoveForward && self.fpsBody.position.y < 150) {
            velocity.y += 0.5;

        } else if (moveForward && lockMoveForward && self.fpsBody.position.y >= 150) {
            velocity.x -= 0.5;
        ...
        //if (moveForward && lockMoveForward && self.fpsBody.position.y < 250) {
        //    self.fpsBody.position.y = self.fpsBody.position.y + 100;

        //} else if (moveForward && lockMoveForward && self.fpsBody.position.y > 250) {
        //    velocity.y -= 1;
        ...
        //} else if (!moveForward && !lockMoveForward && self.fpsBody.position.z > -350) {
        //    self.fpsBody.position.y = 100;

        //} else if (moveForward && !lockMoveForward && self.fpsBody.position.z > -350) {
        //    self.fpsBody.position.y = 100;
      
        //} else if (moveForward && !lockMoveForward && self.fpsBody.position.z < -350) {
        //    self.fpsBody.position.y = 150;
